...
if not exist ".env" (
  ...
  if exist ".env.template" (
    ...
    echo.>> .env
    echo # Note: Workspace directory is now configured as a relative path (../workspace) in the configuration>> .env
    echo.>> .env
...
if not exist "bridge_config.json" (
  echo Creating bridge_config.json from template...
  if exist "bridge_config.json.template" (
    copy bridge_config.json.template bridge_config.json
    echo bridge_config.json created. You'll need to update it with your settings.
  ) else (
    ...
    echo         "./node_modules/@modelcontextprotocol/server-filesystem/dist/index.js",>> bridge_config.json
    echo         "$WORKSPACE_DIR">> bridge_config.json
    echo       ],>> bridge_config.json
    echo       "allowedDirectory": "$WORKSPACE_DIR">> bridge_config.json
    echo     },>> bridge_config.json
    echo     "brave-search": {>> bridge_config.json
    echo       "command": "node",>> bridge_config.json
    echo       "args": [>> bridge_config.json
    echo         "./node_modules/@modelcontextprotocol/server-brave-search/dist/index.js">> bridge_config.json
    echo       ],>> bridge_config.json
    ...
    echo       "command": "node",>> bridge_config.json
    echo       "args": [>> bridge_config.json
    echo         "./node_modules/@modelcontextprotocol/server-sequential-thinking/dist/index.js">> bridge_config.json
    echo       ]>> bridge_config.json
    echo     }>> bridge_config.json
    echo   },>> bridge_config.json
    ...
    echo }>> bridge_config.json
    echo bridge_config.json created with default settings.
  )
) else (
...
echo.
echo Next steps:
echo  1. Update .env file with your API keys (which will be referenced in bridge_config.json)
=======
=======
echo.
